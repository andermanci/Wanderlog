-- ============================================================
-- WANDERLOG - Hora fija: distinguir una cita de un bloque aproximado
-- ============================================================
-- La detección de conflictos daba por hecho que end_time es "la hora a la que me
-- voy". En realidad el usuario lo usa como "más o menos lo que voy a estar ahí",
-- y encadena los bloques: el Coliseo de 09:30 a 10:45 y el Foro de 10:45 a 13:00.
-- Con el hueco a cero, CUALQUIER trayecto (aunque sean 5 minutos andando)
-- disparaba un "no llegas" — y el día entero acababa en rojo, que es no avisar de
-- nada.
--
-- Una actividad tiene HORA FIJA cuando llegar tarde significa PERDERLA, no entrar
-- 13 minutos después. Solo esas generan aviso. Se deduce sola en dos casos (los
-- vuelos y transportes, porque el tren no te espera; y las actividades con una
-- reserva vinculada por documents.activity_id), y esta columna cubre el tercero:
-- la entrada con hora que la app no puede adivinar.
--
-- Por defecto false: quien no ponga horas, o las ponga a ojo, no ve ni un aviso.

alter table public.activities
  add column if not exists fixed_time boolean not null default false;

comment on column public.activities.fixed_time is 'Hora comprometida (entrada o reserva): llegar tarde significa perderla';

-- ------------------------------------------------------------
-- duplicate_trip() copia columna a columna: hay que añadir fixed_time o el viaje
-- duplicado la perdería. Es la TERCERA columna que obliga a tocar esta función
-- (ver 039). Recordatorio para la siguiente.
-- ------------------------------------------------------------
