-- ============================================================
-- WANDERLOG - Duplicar viaje (itinerario completo)
-- ============================================================

-- Duplica un viaje para el usuario actual: datos del viaje, guías del
-- destino, días del itinerario y actividades (remapeando guide_id,
-- day_id, end_day_id y las claves de day_orders a los nuevos ids).
-- NO se duplican gastos, documentos, viajeros, avisos, lugares
-- guardados, diario ni colaboradores: son datos propios de ese viaje
-- concreto ya vivido, no de un plan reutilizable.
...
begin
  ...
  loop
    declare
      v_new_day_id uuid;
    begin
      insert into public.itinerary_days (trip_id, date, notes, guide_id, city)
      values (
