...
export function useTripWeather(trip?: Trip | null, days?: ItineraryDay[], activities?: Activity[]) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Record<string, DayWeather>> => {
      ...
      // Respaldo: geocodificar el destino del viaje (también Open-Meteo).
      let fallback: { lat: number; lng: number } | null = null
      if (days!.some(d => !coordForDay.has(d.date))) {
        try {
          ...
          if (j.results?.[0]) fallback = { lat: j.results[0].latitude, lng: j.results[0].longitude }
        } catch { /* sin respaldo */ }
      }
